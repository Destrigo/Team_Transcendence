...
export class AuthService {
	...
	async register(dto: RegisterDto) {
		...
		if (existing) {
			if (existing.email === dto.email) throw new ConflictException('auth.emailTaken');
			if (existing.username === dto.username) throw new ConflictException('auth.usernameTaken');
		}
		...
		const tokens = await this.generateTokens(user.id, user.email, user.twoFactorEnabled);
	...
	async login(dto: LoginDto) {
		...
		if (!user || !user.passwordHash) throw new UnauthorizedException('auth.invalidCredentials');

		const isMatch = await bcrypt.compare(dto.password, user.passwordHash);
		if (!isMatch) throw new UnauthorizedException('auth.invalidCredentials');

		if (user.twoFactorEnabled) { // if 2FA is active, restrict token drop until 2FA code is verified
	...
	async refreshTokens(dto: RefreshDto) {
		const user = await this.prisma.user.findUnique({ where: { email: dto.email } });
		if (!user || !user.hashedRefreshToken) throw new UnauthorizedException('auth.errors.accessDenied');

		const matches = await bcrypt.compare(dto.refreshToken, user.hashedRefreshToken);
		if (!matches) throw new UnauthorizedException('auth.errors.accessDenied');
	...
	async validateOAuth(provider: string, token: string) {
		...
		let user = await this.prisma.user.findUnique({ where: { email: externalEmail } });
		if (!user) {
	...
	async enable2FA(userId: string, code: string) {
		const user = await this.prisma.user.findUnique({ where: { id: userId } });
		if (!user || !user.twoFactorSecret) throw new BadRequestException('auth.errors.setupNotInitiated');

		const result = await this.otp.verify({ token: code, secret: user.twoFactorSecret });
		if (!result.valid) throw new UnauthorizedException('auth.errors.invalid2fationCode');
