...
export default function AssetDetailsPage() {
  ...
  if (!symbol) {
    return (
      <div className="p-6">
        <h1 className="mb-4 text-lg font-semibold">{t('assetDetails.selectPrompt')}</h1>
        <AssetTable selectedAssetId={null} onSelectAsset={(a) => navigate(`/asset-details/${a.symbol}`)} />
      </div>
    );
  }
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-[1.4fr_1fr]">
        ...
        <div className="rounded-lg border border-border bg-card shadow-sm lg:col-start-2 lg:row-start-2">
          ...
          {assetOrders.length === 0 ? (
            <p className="p-4 text-sm text-muted-foreground">{t('assetDetails.noOrders')}</p>
          ) : (
