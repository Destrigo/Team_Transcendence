...
interface Trade           { filledAt: string; asset: { symbol: string; name: string }; type: string; quantity: number; price: number; total: number; }
        ...
        <>
          ...
          <Section title="Portfolio Performance">
            {portfolioData.length === 0 ? (
              <EmptyState msg="No snapshot data yet — will appear after the first daily snapshot runs at midnight." />
            ) : (
          ...
          <Section title="Asset Allocation">
            {allocation.length === 0 ? (
              <EmptyState msg="No holdings yet — place some trades to see allocation." />
            ) : (
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie data={allocation} dataKey="value" nameKey="symbol" cx="50%" cy="50%" outerRadius={100} label={({ symbol, percentage }) => `${symbol} ${percentage.toFixed(1)}%`}>
                    {allocation.map((_, i) => (
                  ...
                  <tbody>
                    {trades.map((t, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
                        ...
                        <td style={{ ...td, color: t.type === 'buy' ? 'green' : 'red', fontWeight: 600 }}>{t.type.toUpperCase()}</td>
